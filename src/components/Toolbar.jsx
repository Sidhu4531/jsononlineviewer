export default function Toolbar({
  view, onView,
  onFormat, onMinify, onCopy, onClear, onSample, onFile, onTheme, theme, copied,
  samples, indent, onIndent, search, onSearch, typeFilter, onTypeFilter
}) {
  return (
    <header className="toolbar">
      <div className="toolbar-row">
        ...
        <div className="toolbar-actions">
          <button className="action" onClick={onFormat} title="Format (pretty-print)">↔ Format</button>
          <button className="action" onClick={onMinify} title="Minify (remove whitespace)">⇤ Minify</button>
          <button className="action" onClick={onCopy} title="Copy to clipboard">
            {copied ? '✓ Copied' : '⧉ Copy'}
          </button>
          <button className="action" onClick={onFile} title="Open .json file">📁 Open</button>
          <select
            className="action"
          ...
          <button className="action" onClick={onClear} title="Clear input">✕ Clear</button>
          <button className="action theme-btn" onClick={onTheme} title="Toggle theme" aria-label="Toggle theme">
            {theme === 'light' ? '🌙' : '☀️'}
          </button>
        </div>
      </div>
      ...
      <div className="toolbar-row toolbar-row-2">
        ...
        <div className="filter-pills" role="group" aria-label="Filter by type">
          {['all', 'string', 'number', 'boolean', 'null', 'object', 'array'].map((t) => (
            <button
              key={t}
